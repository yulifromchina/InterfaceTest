...
def user_auth(request):
    get_http_auth = request.META.get('HTTP_AUTHORIZATION',b'')  # 格式：BASEC XXXXX
    auth = get_http_auth.split()   # 格式：['BASIC','xxxx']
    if len(auth)<2:
        return "null"
    auth_parts = base64.b64decode(auth[1]).decode().split(':')  #bytes-->str  格式：['username','password]
    uname,pwd=auth_parts[0],auth_parts[1]
    # 使用django的auth
    user = django_auth.authenticate(username=uname,password=pwd)
...
def sec_add_event(request):
    ...
    if auth_result == "null":
        return JsonResponse({'status':10021,'message':'auth cannot be null'})
    if auth_result == "fail":
        return JsonResponse({'status': 10022, 'message': 'auth fail'})
